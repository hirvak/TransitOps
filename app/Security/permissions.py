...
def require_manager(current_user: User = Depends(get_current_active_user)) -> User:
    """
    Dependency to require the user to have the MANAGER role.
    """
    if current_user.role is None or current_user.role.name != ROLE_MANAGER:
        logger.warning(
            f"Permission denied: User {current_user.id} requested MANAGER access but has role "
            f"{current_user.role.name if current_user.role else 'None'}"
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
